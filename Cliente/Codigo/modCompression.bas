...
Public Skins_path As String

Public CambiaSkin As Boolean
...
On Local Error GoTo ErrHandler
    ...
    Select Case file_type
        ...
        Case Skins
            SourceFilePath = App.path & "\Skins\" & CargamosSkinConNombre & ".ias"
            OutputFilePath = Skins_path
...
On Local Error GoTo ErrHandler
    ...
    Select Case file_type
        ...
        Case Skins
            SourceFilePath = Skins_path
            SourceFileExtension = ".bmp"
            OutputFilePath = dest_path & CargamosSkinConNombre & ".ias"
...
On Local Error GoTo ErrHandler
    ...
    'Find the Info Head of the desired file
    If CambiaSkin = True Then
    InfoHead = File_Find(App.path & "\Skins\" & CargamosSkinConNombre & ".ias", File_Name)
    Else
    InfoHead = File_Find(resource_path & "Interface.cao", File_Name)
    End If
    ...
    handle = FreeFile
    If CambiaSkin = True Then
    Open App.path & "\Skins\" & CargamosSkinConNombre & ".ias" For Binary Access Read Lock Write As handle
    Else
    Open resource_path & "Interface.cao" For Binary Access Read Lock Write As handle
    End If
...
Public Function LoadSkins(ByVal FileName As String) As IPictureDisp
...
If hMem <> 0 Then
    ...
    If lpMem <> 0 Then
        ...
        If CreateStreamOnHGlobal(hMem, 1, istm) = 0 Then
            If CLSIDFromString(StrPtr("{7BF80980-BF32-101A-8BBB-00AA00300CAB}"), IID_IPicture(0)) = 0 Then
                Call OleLoadPicture(ByVal ObjPtr(istm), ByteCount, 0, IID_IPicture(0), LoadSkins)
            End If
        End If
    End If
End If
