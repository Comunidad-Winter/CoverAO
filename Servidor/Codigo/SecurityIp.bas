...
Public Function IPSecuritySuperaLimiteConexiones(ByVal ip As Long) As Boolean
    ...
    If IpTableIndex >= 0 Then
        ...
        If MaxConTables(IpTableIndex + 1) < LIMITECONEXIONESxIP Then
            LogIP ("Agregamos conexion a " & ip & " iptableindex=" & IpTableIndex & ". Conexiones: " & MaxConTables( _
                    IpTableIndex + 1))
        ...
        Else
            LogIP ("rechazamos conexion de " & ip & " iptableindex=" & IpTableIndex & ". Conexiones: " & MaxConTables( _
                    IpTableIndex + 1))
