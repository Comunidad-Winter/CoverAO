...
End If
        ...
        If tUser > 0 Then
         ...
         UserList(tUser).Correos(slot).Emisor = "CoverAO Staff"
         Else
         ...
         If EnvioDirecto = 1 Then
         Call WriteConsoleMsg(tUser, "Has recibido una respuesta de [CoverAO Staff], ve a un correo local para leerlo.", FontTypeNames.FONTTYPE_INFO)
         Else
        ...
        If EnvioDirecto = 1 Then
        Call WriteVar(App.Path & "\charfile\" & Destinatario & ".chr", "CORREO", "EMISOR" & slot, "CoverAO Staff")
        Else
