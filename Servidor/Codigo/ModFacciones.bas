...
On Error GoTo EnlistarCaos_Err
       ...
       Matados = (UserList(UserIndex).Faccion.RenegadosMatados + UserList(UserIndex).Faccion.ArmadaMatados + UserList(UserIndex).Faccion.CiudadanosMatados + UserList(UserIndex).Faccion.MilicianosMatados + UserList(UserIndex).Faccion.RepublicanosMatados)
        ...
        If esCaos(UserIndex) Then
            Call WriteChatOverHead(UserIndex, "Ya perteneces a la horda del caos, traeme Mas almas.", str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex))
            Exit Sub
        End If
        ...
        If Matados < 30 Then
            Call WriteChatOverHead(UserIndex, "Para unirte a nuestras fuerzas debes matar al menos 30 enemigos, solo has matado " & Matados, str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex))
            Exit Sub
        End If
...
On Error GoTo EnlistarMilicia_Err
       ...
       Matados = (UserList(UserIndex).Faccion.RenegadosMatados + UserList(UserIndex).Faccion.CaosMatados + UserList(UserIndex).Faccion.ArmadaMatados + UserList(UserIndex).Faccion.CiudadanosMatados)
        ...
        If Matados < 15 Then
            Call WriteChatOverHead(UserIndex, "Para unirte a nuestras fuerzas debes matar al menos 15 enemigos, solo has matado " & Matados, str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex))
            Exit Sub
        End If
